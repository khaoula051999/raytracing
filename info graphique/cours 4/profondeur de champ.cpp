...
class Vecteur
{ // Definition d'une classe pour les vecteurs
public:
...
{ // multiplication d'un vecteur pour un double
...
Vecteur operator/(const Vecteur &a, double b)
{ // division d'un vecteur pour un double
    return Vecteur(a[0] / b, a[1] / b, a[2] / b);
}

Vecteur prodVect(const Vecteur &a, const Vecteur &b)
...
Vecteur prodTermeaterme(const Vecteur &a, const Vecteur &b)
{ // produit terme e terme entre 2 vecteur
    return Vecteur(a[0] * b[0], a[1] * b[1], a[2] * b[2]);
}

Vecteur aleatoire(const Vecteur &N)
{                                // Renvoie un vecteur z*N+x*T1+y*T2, avec (N,T1,T2) un repere, et x,y,z aleatoires suivant une loi de probabilite cosinus
...
class Rayon
{ // Classe pour les rayons (caracterises par leur centre et leur direction)
public:
...
class Sphere
{ // classe pour les spheres (rajout d'un attribut pour savoir si c'est un miroir ou pas, et un autre pour la transparence)
public:
    ...
    bool intersection(const Rayon &r, Vecteur &P, Vecteur &N, double &t)
    {
        ...
        if (delta < 0)
            return false; // pas de solution, donc pas d'intersection

        double racinedelta = sqrt(delta);
        double t2 = (-b + racinedelta) / (2 * a);
        if (t2 < 0)
            return false; // pas de solution positive, donc pas d'intersection
...
class Scene
{ // classe de la scene, qui peut comporter plusieurs spheres
public:
    Scene(){};
    bool intersection(const Rayon &r, Vecteur &P, Vecteur &N, Vecteur &albedo, double &t, bool &miroir, bool &transp, int &id)
    {
        ...
        { // pour chacune des spheres de la scene
            Vecteur Pobjet, Nobjet;
            double tobjet;
            if (objets[i].intersection(r, Pobjet, Nobjet, tobjet) && tobjet < t)
            { // s'il y a une intersection plus proche que celles existantes, alors on prend en compte celle-ci et pas les autres
                intersecte = true;
                t = tobjet;
                P = Pobjet;
                N = Nobjet;
    ...
    Vecteur obtenirCouleur(const Rayon &r, int rebond, bool dernierDiffus)
    { // pour obtenir la couleur
        if (rebond > 5)
            return Vecteur(0., 0., 0.); // si on depasse 5 rebonds, on renvoit la couleur noire
        else
        {
            double t;
            bool miroir, transp;
            ...
            int id;
            if (intersection(r, P, N, albedo, t, miroir, transp, id))
            { // en cas d'intersection avec un objet de la scene
                if (id == 0)
                { // la 1ere sphere correspond e la lumiere
                ...
                {
                    if (transp)
                    { // si  il est transparent

                        double n1 = 1., n2 = 1.4; // indices optiques
                        Vecteur N2 = N;
                        if (prodScal(r.u, N) > 0)
                        { // si on sort de la sphere, on inverse les n et la normale
                            std::swap(n1, n2);
                            N2 = -N;
                        }
                        double angle = 1 - n1 * n1 / (n2 * n2) * (1 - prodScal(r.u, N2) * prodScal(r.u, N2));
                        if (angle < 0)
                        {                                                                // si l'angle est plus petit que 0, il y a reflexion
                            Vecteur Directionreflechie = r.u - 2 * prodScal(r.u, N) * N; // direction de la reflexion
                            Rayon Rayonreflechi(P + 0.00001 * N, Directionreflechie);    // rayon reflechi, partant du point d'intersection et allant dans la direction reflechie
                            return obtenirCouleur(Rayonreflechi, rebond + 1, false);
                        }
                    ...
                    {
                        // eclairage direct
                        Vecteur w = aleatoire((P - L).normaliser()); // Vecteur qui a plus de chance d'etre dirige vers la source de lumiere.
                        Vecteur xp = w * objets[0].R + objets[0].O;
                        Vecteur Pxp = xp - P;
                        ...
                        if (intersection(Rayonombre, Pombre, Nombre, albedoombre, tombre, miroirombre, transombre, idombre) && tombre < normePxp - 0.0001)
                        {                                  // s'il y a intersection avant d'arriver e la source de lumiere
                            couleur = Vecteur(0., 0., 0.); // pas eclaire = ombre
                        }
                        else
                        {
                            couleur = I / (4 * M_PI * M_PI * objets[0].R * objets[0].R) * albedo / M_PI * std::max(0., prodScal(N, Pxp)) * std::max(0., -prodScal(w, Pxp)) / (normePxp * normePxp) / (std::max(0., -prodScal((L - P).normaliser(), w)) / (M_PI * objets[0].R * objets[0].R));
                        }
...
int main()
{
    ...
    Sphere Slumiere(scene.L, 5, Vecteur(1., 0.3, 0.2));
...
#pragma omp parallel for schedule(dynamic, 1)
    for (int i = 0; i < H; i++)
    {
        for (int j = 0; j < W; j++)
        {
            ...
            for (int k = 0; k < nmbrayon; k++)
            {
                ...
                Vecteur Cp = C + Vecteur(x3, x4, 0);    // point d'origine aleatoire dans le disque d'ouverture
    ...
    stbi_write_png("image_profondeurchamp.png", W, H, 3, &image[0], 0);
