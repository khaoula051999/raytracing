...
class Vecteur{ //Definition d'une classe pour les vecteuras
public:
...
int main() {
	...
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
            ...
            if (S.intersection(r,P,N)) { //en cas d'intersection avec la sphere, on calcule la couleur du pixel
	...
	stbi_write_png("image_lumiere.png", W, H, 3, &image[0], 0);
