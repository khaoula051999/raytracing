...
public:
    Scene() {};
    bool intersection(const Rayon& r, Vecteur& P, Vecteur& N, Vecteur &albedo, double &t){
        t =1E9;
...
int main() {
	...
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
            ...
            if (scene.intersection(r,P,N,albedo,t )) {// en cas d'intersection avec un objet de la scene
                ...
                if (scene.intersection(Rayonombre, Pombre, Nombre, albedoombre, tombre) && tombre < normePL) { //s'il y a intersection avant d'arriver e la source de lumiere
	...
	stbi_write_png("ombreportee.png", W, H, 3, &image[0], 0);
