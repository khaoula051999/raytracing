...
class Sphere{ //classe pour les spheres (rajout d'un attribut pour savoir si c'est un miroir ou pas)
public:
...
public:
    ...
    Vecteur obtenirCouleur(const Rayon& r, int rebond) { //pour obtenir la couleur
        if (rebond > 5) return Vecteur(0.,0.,0.); // si on depasse 5 rebonds, on renvoit la couleur noire
            ...
            if (intersection(r,P,N,albedo,t,miroir)) { // en cas d'intersection avec un objet de la scene
                if (miroir) { // si c'est un miroir
					...
					if (intersection(Rayonombre, Pombre, Nombre, albedoombre, tombre,miroirombre) && tombre < normePL) {//s'il y a intersection avant d'arriver e la source de lumiere
