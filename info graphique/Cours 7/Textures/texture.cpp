...
Vecteur operator/(double b, const Vecteur& a){
    return Vecteur(b/a[0], b/a[1], b/a[2]);
}
...
public:
    Object(){};
    virtual bool intersect(const Ray& r, Vecteur& P, Vecteur& N, double& t, Vecteur &color) = 0;
...
public:
  ~TriangleMesh() {}
    ...
    void buildBVH(Noeud* n, int debut, int fin){
        ...
        Vecteur diag = n->b.maxi - n->b.mini;
        int dim = 0;
    ...
    bool intersect(const Ray& r, Vecteur& P, Vecteur& normal, double& t, Vecteur &color){
        ...
        while(!l.empty()){
            ...
            if (courant->fg){
                ...
                for (int i = courant->debut; i < courant->fin; i++){
                    ...
                    Vecteur AC = r.C - A;
                    Vecteur AC_cross_u = cross(AC, r.u);
                    double invU_cross_N = 1./prodScal(r.u, N);
                    double beta = - prodScal(e2, AC_cross_u)*invU_cross_N;
                    double gamma = prodScal(e1, AC_cross_u)*invU_cross_N;
                    double alpha = 1 - beta - gamma;
                    double localt = -prodScal(AC, N)*invU_cross_N;
...
*/                          normal = alpha*normals[indices[i].ni] + beta*normals[indices[i].nj] + gamma*normals[indices[i].nk];
                            ...
                            if (uvy < 0) uvy += H;
                            uvy = H - uvy - 1;
                            color = Vecteur(std::pow(textures[indices[i].group][(uvy*H + uvx)*3]/255., 2.2),
...
class Sphere : public Object {
    public:
        ...
        bool intersect(const Ray& r, Vecteur& P, Vecteur& N, double& t, Vecteur &color){
            double a = 1;
...
public:
    ...
    bool intersect(const Ray& r, Vecteur& P, Vecteur& N, Vecteur &albedo, bool &mirror, bool &transp, double &n, double &t, int &objectId){
        ...
        for(int i = 0; i < objects.size(); i++){
            Vecteur localP, localN, localAlbedo;
            double localt;
...
int main() {
    ...
    Sphere Lumiere(scene.L, 2, Vecteur(1., 1., 1.));
